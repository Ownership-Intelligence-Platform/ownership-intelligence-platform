...
@app.get("/entities/{entity_id}/guarantees")
def api_get_entity_guarantees(entity_id: str, direction: str = "out"):
    """List guarantee relationships for an entity.

    direction: out | in | both (guarantor / guaranteed / either).
    """
    items = get_guarantees(entity_id, direction=direction)
    return {"entity": {"id": entity_id}, "count": len(items), "items": items}


@app.get("/entities/{entity_id}/supply-chain")
def api_get_entity_supply_chain(entity_id: str, direction: str = "out"):
    """List supply chain relationships for an entity.

    direction: out | in | both (supplier / customer / either).
    """
    items = get_supply_chain(entity_id, direction=direction)
    return {"entity": {"id": entity_id}, "count": len(items), "items": items}


@app.get("/entities/{entity_id}/employment")
def api_get_entity_employment(entity_id: str, role: str = "both"):
    """List employment SERVES_AS relations for an entity.

    role: as_company | as_person | both.
    """
    items = get_employment(entity_id, role=role)
    return {"entity": {"id": entity_id}, "count": len(items), "items": items}


@app.get("/entities/{entity_id}/locations")
def api_get_entity_locations(entity_id: str):
    """Return grouped locations for an entity."""
    locs = get_locations(entity_id)
    # Expand into flat list for symmetry with other endpoints while preserving grouping
    items = [
